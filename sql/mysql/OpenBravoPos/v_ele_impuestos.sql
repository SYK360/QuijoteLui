CREATE OR REPLACE VIEW `v_ele_impuestos` AS
    SELECT 
        ...
        SUM(CAST(abs(((tl.UNITS * tl.PRICE) * (tx.RATE + 1)))
    ...
    GROUP BY CAST(CONCAT(t.TICKETID, tl.LINE) AS UNSIGNED) , CAST(if(t.tickettype = 0, 'FV', 'NCV') AS CHAR (10) ) , CAST(CONCAT(FUN_ESTABLECIMIENTO(),
                FUN_PUNTO_EMISION(),
                LPAD(t.TICKETID, 9, '0'))
